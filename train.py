...
if config.logging == "wandb":
    run = wandb.init(project="VAE", dir=config.get_base_dir(), config=config)
...
for e in trange(1, epochs + 1, dynamic_ncols=True):
    ...
    tqdm.write(
        f"[TRAIN] Epoch: {e:3d}| Overall Loss: {train_loss:.2f}| Reconstruction Loss: {train_loss_rc:.2f}| KL Divergence: {train_loss_kl:.2f}"
    )
    if config.logging == "wandb":
        run.log(
            {
                ...
                "Epochs": e,
                "Random Sampling": wandb.Image(random_sampling(vae,2,28,5,device)),
                "Manifold": wandb.Image(plot_manifold(vae,[-2,2],[-2,2],28,12,device)),
                "Linear Interpolation": wandb.Image(plot_linear_interpolation(vae,[-2,2],[2,-2],28,25,device))
            }
        )
